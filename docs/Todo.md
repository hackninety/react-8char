...
### ✅ 校对时辰·定盘（2026-08，参照 react-zwds rectify）
[`rectify.ts`](../src/lib/rectify.ts) + [`RectifyPanel`](../src/components/RectifyPanel.tsx)：时辰不详时同日
十三时辰（早子~晚子）并排起盘（按标准时辰、不作真太阳时校正），差异列=四柱/时柱十神/起运；
十条性格特征按时柱十神/时支作用/羊刃桃花驿马判定（十三盘年月日柱恒同，判定聚焦随时辰而变的部分），
勾选命中×1 + 已发生大事年份 K线反查×2 打匹配分排序。晚子时跨日重点标注：正统派日柱进次日、
整盘日主随变（探针实证两派时柱同为次日子时干、日柱随 sect 异）；选定后回填代表时刻并关真太阳时直接起盘。
时柱与流时表 getLiuShiForDay 十三格互证（排盘引擎 × ext 两路径），期望值冻结 [`rectify.test.ts`](../tests/rectify.test.ts) 7 例。
...
随 JSON（紧凑 rows）与 MD（表格）导出。④ **能量多边形** [`wuxing-energy.ts`](../src/lib/wuxing-energy.ts) +
